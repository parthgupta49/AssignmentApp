...
exports.changePassword = async (req, res) => {
    ...
    try {
        ...
        if (!updatedUser) {
            return res.status(500).jsoen({ success: false, message: "Failed to update password." });
        }

        console.log(updatedUser);
        await mailSender(updatedUser.email, "Password Changed", 
            // `Password changed for ${updatedUser.firstname + " " + updatedUser.lastname}`
