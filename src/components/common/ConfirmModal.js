...
const ConfirmModal = ({ modalData }) => {
    return (
        <div className='fixed inset-0 flex items-center justify-center z-50 bg-richblack-700 backdrop-blur-sm bg-opacity-50'> {/* Added overlay */}
            <div className='bg-gray-800 py-5 px-6 rounded-md text-gray-300 border border-gray-300'>
                <div className='flex flex-col gap-2'>
                    ...
                    <div className='flex gap-x-3'>
                        <IconBTN
                            ...
                            customClasses={`px-2 py-1  rounded-md bg-yellow text-gray-900 font-medium ${modalData?.btn1CustomCSS}`}
                        />
                        <button
                            onClick={modalData.btn2Handler}
                            className={`px-2 py-1  rounded-md bg-gray-100 text-gray-800 font-medium ${modalData?.btn2CustomCSS}`}
                        >
                            {modalData.btn2Text}
                        </button>
                    </div>
